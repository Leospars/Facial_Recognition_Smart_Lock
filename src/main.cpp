  ...
  if (Serial.available()) {
    ...
    if (!error) {
      ...
      if (status == "match") {
        ...
        bootTime = (millis() - k230StartTime) / 1000;
        serverLog("{\"event\": \"boot\", \"bootTime\": \"" + String(bootTime) + "\"}");
      }
    }
  }
}
  ...
  if (millis() - lastBatCheck > 15 * 60000ULL) {  // Check every 15 minutes
    ...
    switch (batLevel) {
      ...
      case 0:
        ...
        break;
      default:
        FCM_Notification("Lock Battery", "{\"battery\": " + String(batLevel) + "%}");
    }
    lastBatCheck = millis();
  }
}
...
void initialCommisioning() {
  ...
  bleServer.sendResponse("{\"lock_id\":\"" + String(LOCK_ID) + "\",\"lock_ip\":\"" + WiFi.localIP().toString() + "\",hostname\":\"" + String(WiFi.getHostname()) + "\"}");
...
void mqttCallback(char *topic, byte *payload, unsigned int length) {
  ...
  else if (doc["cmd"] == "start_call") wakeK230D("{\"cmd\":\"start_call\", \"room_id\":\"" + doc["room_id"].as<String>() + "\"}");
